...
## Retrieval
- **k / limit** = `5`. Why: `The value of k directly controls how much retrieved context is injected into the LLM prompt. If k is too large (e.g., k=20), the prompt becomes overloaded with loosely relevant documents, increasing noise and reducing answer quality due to context dilution and higher risk of irrelevant or conflicting information. If k is too small, we risk missing relevant documents. Therefore, k=5 provides a balance between semantic coverage and maintaining a compact, high-signal context window for the LLM.`
